...
@CrossOrigin
@RestController
@RequestMapping(value = "/workLogEdit")
public class ProjectLogEditController {
    private static Logger logger = LoggerFactory.getLogger(ProjectLogEditController.class);
    ...
    @OperLog(operType = OperLogType.SAVE,operDesc = "审核结构化日志明细")
    ...
    @OperLog(operType = OperLogType.QUERY,operDesc = "获取日志审核，修改界面字典(角色)，(类型)数据")
    ...
    @OperLog(operType = OperLogType.QUERY,operDesc = "查询项目")
    ...
    @OperLog(operType = OperLogType.QUERY,operDesc = "获取项目日志修改-计划树")
    public OfficeResopnseBean getProjectPlanTree(@RequestParam String contractNo, @RequestParam String projectId, @RequestParam Integer part) {
        Map<String, Object> map = projectPlanManager.getProjectPlanQueryTree(contractNo,projectId,part);
        OfficeResopnseBean responseBean = OfficeResopnseBean.newSuccessBean();
        responseBean.setBody(map);
        logger.info("获取项目日志修改-计划树成功！[{}]", JSONUtils.toString(map));
        return responseBean;
    }
}
